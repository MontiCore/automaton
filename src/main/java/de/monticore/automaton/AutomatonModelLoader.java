...
public class AutomatonModelLoader extends ModelingLanguageModelLoader<ASTAutomaton> {
  ...
  protected void createSymbolTableFromAST(final ASTAutomaton ast, final String modelName,
    ...
    final AutomatonSymbolTableCreator symbolTableCreator = getModelingLanguage().getSymbolTableCreator
        (resolverConfiguration, enclosingScope).orNull();

    if (symbolTableCreator != null) {
      Log.info("Start creation of symbol table for model \"" + modelName + "\".", AutomatonModelLoader.class
          .getSimpleName());
